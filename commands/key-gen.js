...
module.exports = {
    data: new SlashCommandBuilder()
		.setName(`${settings.slash_key_gen}`)
        .setDescription(`${settings.descption_key_gen}`)
        .addStringOption((option) =>
        option
            .setRequired(true)
            .setName("product")
            .setDescription("Specify the product you want")
        )
        .addStringOption((option) =>
        option
            .setRequired(true)
            .setName("userid")
            .setDescription("Insert the userid to generate the product")
        ),
    async execute(interaction) {
        ...
        fetch(`${settings.linkapi}?ownerapi=${settings.ownerapi}&type=KeyGen&serverid=${guild}&product=${product}&userid=${userid}&format=text`)
		.then(res => res.text())
		.then(text => {
			if(!text.includes("message"))
			{
				interaction.editReply({ 
					embeds: [new Discord.MessageEmbed()
						...
						.setTitle(`__${settings.NameBOT} - Key Gen__`)
                        .setDescription(`**\n${text}\n**`)
